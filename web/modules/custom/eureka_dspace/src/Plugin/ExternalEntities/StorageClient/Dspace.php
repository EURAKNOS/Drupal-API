...
namespace Drupal\eureka_dspace\Plugin\ExternalEntities\StorageClient;
...
/**
 * External entities storage client based on a DSpace API.
 *
 * TODO: This is a work in progress. We stepped away from using the ExternalEntities module
 *  and use a migration instead. But it can change back to ExternalEntities in the future.
 *  Therefore this and anything else that might be related to ExternalEntities should only be
 *  deleted when we're sure we won't need it anymore.
 *
 * @ExternalEntityStorageClient(
 *   id = "dspace",
 *   label = @Translation("Dspace"),
 *   description = @Translation("Retrieves entities from a DSpace API.")
 * )
 */
class Dspace extends ExternalEntityStorageClientBase implements PluginFormInterface {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['auth'] = [
      ...
      '#description' => 'Authentication using form params.'
    ];

    $form['auth']['endpoint'] = [
      '#type' => 'textfield',
      '#title' => $this->t('User auth endpoint'),
    ...
    $form['auth']['user'] = [
      '#type' => 'textfield',
      '#title' => $this->t('User'),
      '#default_value' => $this->configuration['auth']['user'],
    ];
  ...
  public function load($id) {
    ...
    try {
      ...
      \Drupal::logger('eureka_dspace')->error($e);
    }
  ...
  protected function mapCleanData($data) {
    $clean_data = [];

    $clean_data['id'] = $data['id'];
    dump($data['metadata']['knowledgeobject.title'][0]['value']);
    $clean_data['label'] = $data['metadata']['knowledgeobject.title'][0]['value'];
  ...
  protected function authenticate() {
    ...
    try {
      $response = $this->httpClient->request(
        'POST',
        $this->configuration['auth']['endpoint'], [
          'form_params' => [
